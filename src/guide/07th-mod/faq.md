...
2. **禁用、卸载或结束与计算机上安装的任何额外音频增强相关软件或驱动程序相关的进程！**  
    ...
    - 尝试关闭每一个程序，然后使用任务管理器结束计算机上的所有无关进程 (能被结束的非 Windows 关键进程)。然后尝试运行游戏，看看是不是其中某个进程引起的问题。

3. **游戏路径** (游戏在硬盘上的位置) 里包含有特殊字符也可能会导致游戏无法读取音频文件, 但在播放开场动画视频的时候是能听到声音的：

    - 想测试是不是这个问题导致的，可以试着把游戏移动到下面提到的简单路径。即便是你觉得路径不存在问题，也可以执行这个操作试试。

      - 在 Linux 或是 MacOS  系统上，试着将游戏移动到 ```/home/用户名/higurashi```
      - 在 Windows  系统上，试着将游戏移动到 ```C:\temp\higurashi```
    
    - 针对 Linux 系统：

      - 在某些章节中 (例如《第九章：礼》)，据说即使路径中包含了空格 (``` ```) 也会出现问题，所以请试着尽可能清理掉特殊字符 (只保留字母、数字和下划线) - 可以参考 [Github 上的问题部分 - 寒蝉《第九章：礼》 (英文)](https://github.com/07th-mod/higurashi-rei/issues/27#issuecomment-2760403144)

    - 针对 Windows 系统，空格和点号 (```.```) 应该是没问题的，但也请检查下：
      
      - 特殊符号，例如井号 (```#```)，加号 (```+```) 之类的符号。
      - 非英文字符 (例如中文汉字)
      - 在游戏路径中包含的其他奇怪的特殊字符。

4. 检查一下其他章节的游戏有没有问题，还是只有这一个章节有问题。目前已知如下章节有特别的问题：

    - [Linux 系统 - 《第九章：礼》] 游戏路径中的空格 (可能也包含其他特殊字符) 会导致音频无法加载，但在播放开场动画视频的时候是能听到声音的
...
![](/guide/device-manager-frescologic-driver.webp)  

## 已知问题 - 特定的章节或版本

在此我们记录了某些版本目前已知的问题，例如只有《第五章》或 MangaGamer 官网的 Unix 版本游戏才会遇到的问题。
...
### 玩《第八章：祭囃篇》时，阅读完最后一条 Tip 遇到 “追加内容” 菜单选项缺失的问题

**2025 年 1 月 12 日更新**: **这个问题应该已经解决了，请更新下你的补丁**。想要了解更多信息，请查看[《第八章》版本 v2.3.2 更新说明](https://github.com/07th-mod/matsuribayashi/releases/tag/v2.3.2)。如果你仍然遇到了这个问题，请查看下方折叠的 “原先的解决方案”。

::: details 原先的解决方案

《第八章：祭囃篇》自 2020 年起，长久以来都存在一个问题。就是在阅读完最后一条 Tip 后，会导致 “追加内容” 菜单中的选项缺失，例如 “工作室杂谈” 和 “Tips” 选项。

这个问题有两种解决方法：

- **方法一**：使用 F10 菜单强制清除游戏进度（2024 年 1 月 11 日验证仍然有效）
    1. 按下 F10 键
    2. 点击 “疑难解答” 标签页
    3. 在 “实验性的游戏工具” 一栏，点击 “重置游戏进度” 按钮三次。游戏的进度现在应该会被清除。
    4. 重启游戏

- **方法二**：使用 “章节跳跃” 功能跳转到游戏的最后一个章节，然后使用快进功能解锁重新解锁这个 Tip。

如果你想提交任何反馈，或者查看这个问题目前的解决进度，请查看 Github 的问题部分 ["Extras menu locking itself after completion (#26)" (英文)](https://github.com/07th-mod/matsuribayashi/issues/26)。
